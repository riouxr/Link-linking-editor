...
class LL_OT_ShadowLink(bpy.types.Operator):
    ...
    bl_description = (
        "For each selected light, create (or use an existing) shadow linking "
        "blocker collection and add the selected meshes to it, storing the "
        "group name in a custom property on the light."
    )
    ...
    def execute(self, context):
        ...
        # For each light, create or retrieve the shadow‐linking collection
        for light in selected_lights:
            # Make this light active (just in case you need that context):
            bpy.ops.object.select_all(action='DESELECT')
            light.select_set(True)
            context.view_layer.objects.active = light

            # Construct or retrieve the shadow‐link collection
            group_name = light.get("shadow_linking_receiver_collection") or f"Shadow Linking for {light.name}"
            new_group = bpy.data.collections.get(group_name)
            if not new_group:
                # Create the new collection
                new_group = bpy.data.collections.new(group_name)
                context.scene.collection.children.link(new_group)

            # Store the name in the light’s custom property
            light["shadow_linking_receiver_collection"] = group_name

            # Link all selected meshes that are not already in the collection
            linked_meshes = 0
            for obj in all_meshes:
                if obj.name not in new_group.objects:
                    new_group.objects.link(obj)
...
class LL_OT_ShadowUnlink(bpy.types.Operator):
    ...
    bl_description = (
        "For each selected light, remove the objects (from the Mesh and Collection lists) "
        "that are linked via the shadow linking blocker collection and clear the linking property."
    )
    ...
    def execute(self, context):
        ...
        for light in selected_lights:
            group_name = light.get("shadow_linking_receiver_collection")
            if not group_name:
            ...
            removed = 0
            for obj in list(linking_group.objects):
                # Unlink only if the object's name appears in the selected mesh list.
                if obj.name in [m_obj.name for m_obj in scene.ll_mesh_items if m_obj.selected]:
                    try:
                        linking_group.objects.unlink(obj)
                        print(f"DEBUG: Unlinked {obj.name} from {group_name}")
                        removed += 1
                    except Exception as e:
                        print(f"DEBUG: Error unlinking {obj.name}: {e}")

            total_removed += removed
            # Remove the shadow linking property from the light after unlinking.
            if "shadow_linking_receiver_collection" in light:
                del light["shadow_linking_receiver_collection"]
